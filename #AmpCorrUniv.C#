...
void AmpCorrUniv(const char * filename){
  ...
  rymax_l=mean1+0.05*rms1;
  rymin_r=mean2-1.1*rms2;
  rymax_r=mean2+0.05*rms2;
    
  

  tymin=mean3-1.1*rms3;
  tymax=mean3+0.05*rms3;
  ...
  */
  ...
  tymin_c=tymin-hyp_t->Eval(0.25)+hyp_t->GetParameter(0);
  tymax_c=tymax-hyp_t->Eval(0.25)+hyp_t->GetParameter(0);
  ...
  TH2F* hc_t= new TH2F("hc_t", "histo hc_t",nbinx,txmin,txmax,nbiny,tymin_c,tymax_c);
   ...
   for(k=0;k<digiTree->GetEntries();k++){
	...
	hc_t->Fill((time[1+LEDi]-time[2+LEDi]-(hyp_r->Eval(amp_max[3]/max)-hyp_r->GetParameter(0)-hyp_l->Eval(amp_max[4]/max)+hyp_l->GetParameter(0))),(time[1+LEDi]+time[2+LEDi])/2-time[0]-(hyp_r->Eval(amp_max[3]/max)-hyp_r->GetParameter(0)+hyp_l->Eval(amp_max[4]/max)-hyp_l->GetParameter(0))/2);
